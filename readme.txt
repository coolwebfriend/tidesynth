...
The seven-digit code at the end of the URL represents a NOAA station code. In this case, 9410660 represents the Los Angeles station. You can search for another station at the URL provided. If that doesn’t work look for a station at https://tidesandcurrents.noaa.gov and replace 9410660 with your chosen station code.

Copy/paste the full contents of the harmonic constituent table into Numbers or Excel, and export the data as a CSV suing the same column order and headers as the example CSV provided. 


2. RUN THE SCRIPT

TideSynth is designed to be run from Terminal. Type Python3 followed by the local file path of TideSynth.py. You will be prompted to enter four pieces of information:

  Input File Path: the file path of the CSV you created in step 2.
  Fundamental frequency in Hertz. For musical results, remember that typical human hearing range is 20-20000Hz.
  Audio Duration in Seconds: The length of the file that will be created. 
  Output Filepath: where you’d like to save the audio file, including the name of the file and .wav extension.
